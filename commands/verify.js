...
class cmd extends Base_Command{
    ...
    async execute(bot, member, message, args){
            ...
            if(!discorduser.length) {message.author.send(`Please run the command \`$pair <accountname>\` first.`); return;}
            ...
            if(last.act.data.token === discorduser[0].token ){
                ...
                message.author.send('I added the "Registered Member" role to your account.');
            }
            else{
